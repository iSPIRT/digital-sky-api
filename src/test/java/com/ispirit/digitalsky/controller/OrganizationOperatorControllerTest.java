...
@WebMvcTest(value = OrganizationOperatorController.class, secure = false)
@Import({TestContext.class})
public class OrganizationOperatorControllerTest {
    ...
    public void shouldCreateOperatorProfile() throws Exception {
        ...
        OrganizationOperator operator = new OrganizationOperator(
                0, "DEFAULT", "io1", "email@test.com", "mobile", "contact", "India", asList(address));
        operator.setId(1);

        when(organizationOperatorService.createNewOperator(any())).thenReturn(operator);

        //when
        MockHttpServletResponse response = mvc
                .perform(
                        post("/api/orgOperator")
        ...
        verify(organizationOperatorService).createNewOperator(argumentCaptor.capture());
    ...
    public void shouldHandleExistingProfileError() throws Exception {
        ...
        when(organizationOperatorService.createNewOperator(any())).thenThrow(new OperatorProfileAlreadyExist());
    ...
    public void shouldUpdateOperatorProfileIfFound() throws Exception {
        ...
        OrganizationOperator current = new OrganizationOperator(
                userPrincipal.getId(), "DEFAULT", "io1", "email@test.com", "mobile", "contact", "India", asList(address));
        current.setId(1);

        OrganizationOperator operator = new OrganizationOperator(
                0, "DEFAULT", "io1", "email@test.com", "mobile", "contact", "India", asList(address));

        when(organizationOperatorService.find(1L)).thenReturn(current);

        //when
        MockHttpServletResponse response = mvc
                .perform(
                        put("/api/orgOperator/1")
                                .content(objectMapper.writeValueAsString(operator))
                                .contentType(MediaType.APPLICATION_JSON)
                ).andReturn().getResponse();
        ...
        ArgumentCaptor<OrganizationOperator> argumentCaptor = ArgumentCaptor.forClass(OrganizationOperator.class);
        verify(organizationOperatorService).updateOperator(eq(1L), argumentCaptor.capture());
    ...
    public void shouldNotAllowUnAuthorizedUpdate() throws Exception {
        ...
        OrganizationOperator current = new OrganizationOperator(
                userPrincipal.getId()+1, "DEFAULT", "io1", "email@test.com", "mobile", "contact", "India", asList(address));
        current.setId(1);
        ...
        verify(organizationOperatorService, never()).updateOperator(eq(1L), any());
    }
    ...
    public void shouldNotUpdateOperatorProfileIfNotFound() throws Exception {
        ...
        when(organizationOperatorService.find(1L)).thenReturn(null);
    ...
    public void shouldGetOperatorProfileIfFound() throws Exception {
        ...
        when(organizationOperatorService.find(1L)).thenReturn(operator);

        //when
        MockHttpServletResponse response = mvc
                .perform(
                        get("/api/orgOperator/1")
                ).andReturn().getResponse();

        assertThat(response.getStatus(), is(HttpStatus.OK.value()));
        OrganizationOperator responseBody = objectMapper.readValue(response.getContentAsString(), OrganizationOperator.class);
